...
import {
    ...
    getComplaintDetails
} from "@/scripts/GetALL/GetAllComplaints";
import {
    fetchCategoriesName
} from "@/scripts/GetALL/ComplaintCategoryService";
import { getUserIdByUsername } from "@/scripts/GetALL/GetUserId";
import { getAllComplaintsbyuser } from "@/scripts/GetALL/GetComplaintBy";
...
export default {
    ...
    computed: {
        filteredComplaints() {
            return this.complaints.filter(complaint => complaint.description.includes(this.searchQuery));
        },
    },
    ...
    async mounted() {
        ...
        if (username) {
            ...
            try {
                this.userId = await getUserIdByUsername(username);
    ...
    methods: {
        ...
        async fetchComplaints() {
            if (!this.userId) {
                console.error("User ID is required to fetch complaints.");
                return;
            }
            try {
                const complaintsData = await getAllComplaintsbyuser(this.userId, this.currentPage, this.pageSize);
